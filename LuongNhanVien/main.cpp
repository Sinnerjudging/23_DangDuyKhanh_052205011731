...
bool isValidNgayVaoLam(const std::string& ngayVaoLam) {
	// Ki?m tra ?? dài c?a chu?i
	if (ngayVaoLam.size() != 10) {
		return false;
	}

	// Ki?m tra các ký t? t?i các v? trí c? th? trong chu?i
	if (ngayVaoLam[2] != '/' || ngayVaoLam[5] != '/') {
		return false;
	}


	// Ki?m tra các ký t? s? trong ngày, tháng, n?m
	...
	// So sánh ngày v?i th?i gian hi?n t?i
	tm now_tm;
...
bool isValidPhoneNumber(const std::string& soDienThoai) {
	// Ki?m tra ?? dài c?a chu?i Ki?m tra ký t? ??u tiên có ph?i là s? 0
	if (soDienThoai.length() != 10 || soDienThoai[0] != '0') {
		return false;
	}

	// Ki?m tra các ký t? còn l?i có ph?i là s?
